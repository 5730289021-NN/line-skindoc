...
function handleImage(message, replyToken) {
  const downloadPath = path.join("./", "downloaded", `pic.jpg`);
  ...
  return downloadContent(message.id, downloadPath).then(downloadPath => {
    let content;
      ...
      request.post(
        {
          ...
          url:
            "https://southcentralus.api.cognitive.microsoft.com/customvision/v2.0/Prediction/1f32e321-220b-4dca-a709-5be05765d185/image?iterationId=f020026a-5afd-4cd1-a2e6-fe363063caad",
          body: content
        },
        function(error, response, body) {
          body = JSON.parse(body);
          let output =
            body.predictions[0].tagName +
            " " +
            body.predictions[0].probability * 100 +
            "\n" +
            body.predictions[1].tagName +
            " " +
            body.predictions[1].probability * 100;
